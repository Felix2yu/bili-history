...
<template>
  <div class="space-y-6">
    ...
    <div v-if="viewingData" class="text-base text-center text-gray-600 dark:text-gray-300">
      共收藏了 <span class="text-[#fb7299] font-bold">{{ viewingData.total_count || 0 }}</span> 个视频，
    ...
    <div v-if="viewingData" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">收藏夹分布</h4>
        <div class="space-y-2">
          <div v-for="(folder, index) in viewingData.folder_dist" :key="index"
            ...
            <div class="flex items-center ml-2">
              ...
              <span class="text-sm font-medium text-[#fb7299]">{{ folder.count }}</span>
            </div>
          </div>
      ...
      <!-- Top 创作者 -->
      <div class="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">最爱收藏的UP主</h4>
        <div class="space-y-2">
      ...
      <div class="lg:col-span-2 bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="text-center">
            <div class="text-2xl font-bold text-[#fb7299]">{{ Math.round(viewingData.avg_play_count || 0) }}</div>
