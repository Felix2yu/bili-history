...
<template>
  <div class="space-y-6">
    ...
    <div v-if="viewingData" class="text-base text-center text-gray-600 dark:text-gray-300">
      共点赞了 <span class="text-[#fb7299] font-bold">{{ viewingData.total_count || 0 }}</span> 个视频
    </div>
    ...
    <div v-if="viewingData" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">最爱点赞的UP主</h4>
        <div class="space-y-2">
          <div v-for="(creator, index) in viewingData.top_creators" :key="index"
            ...
            <div class="flex items-center">
              <span class="w-6 h-6 rounded-full bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] flex items-center justify-center text-white text-xs font-bold mr-2">
                {{ index + 1 }}
              </span>
              <span class="text-sm text-gray-700 dark:text-gray-300">{{ creator.name }}</span>
            </div>
            <span class="text-sm font-medium text-[#fb7299]">{{ creator.count }} 个</span>
          </div>
      ...
      <!-- 分类分布 -->
      <div class="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">点赞分类分布</h4>
        <div class="space-y-2">
      ...
      <!-- 时长分布 -->
      <div class="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">点赞视频时长分布</h4>
        <div class="space-y-2">
