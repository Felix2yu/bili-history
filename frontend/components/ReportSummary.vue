  ...
  <div class="space-y-4">
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-rose-500">{{ summary.total_videos }}</div>
        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">观看视频数</div>
      </div>
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-amber-500">{{ formatDurationShort(summary.total_duration) }}</div>
        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">总观看时长</div>
      </div>
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-emerald-500">{{ summary.unique_days }}</div>
        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">活跃天数</div>
      </div>
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-sky-500">{{ summary.unique_authors }}</div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      <div class="glass-card p-4 flex items-center justify-between text-sm">
        ...
        <div class="flex gap-4">
          <span class="text-gray-700 dark:text-gray-300">
            <span class="font-semibold text-rose-500">{{ summary.avg_daily_videos?.toFixed(1) }}</span> 个视频
          </span>
          <span class="text-gray-700 dark:text-gray-300">
            <span class="font-semibold text-amber-500">{{ formatDurationShort(summary.avg_daily_duration) }}</span>
          </span>
        </div>
      </div>
      ...
      <div v-if="summary.completion_stats" class="glass-card p-4 flex items-center justify-between text-sm">
        ...
        <div class="flex gap-4">
          <span class="text-gray-700 dark:text-gray-300">
            <span class="font-semibold text-emerald-500">{{ summary.completion_stats.finished }}</span> 看完
          </span>
          <span class="text-gray-700 dark:text-gray-300">
            <span class="font-semibold text-sky-500">{{ (summary.completion_stats.avg_rate * 100).toFixed(0) }}%</span> 平均完播率
          </span>
        </div>
      </div>
      ...
      <div v-if="summary.device_dist && Object.keys(summary.device_dist).length > 0" class="glass-card p-4 flex items-center justify-between text-sm">
        ...
        <div class="flex gap-3 flex-wrap">
          ...
          >
            ...
            <span class="font-semibold" :style="{ color: getDeviceColor(device) }">{{ count }}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div v-if="summary.daily_breakdown?.length" class="glass-card p-4">
        ...
        <div style="height: 100px;">
          <div class="flex items-end gap-1 h-full">
            <div
              v-for="(day, idx) in summary.daily_breakdown"
              :key="day.date"
            ...
            >
              ...
              <div class="w-full mt-auto rounded-t transition-all duration-300 hover:opacity-80"
                   :style="{ height: `${Math.max((day.count / maxDailyCount) * 100, 4)}%`, backgroundColor: getChartColor(idx) }"
              ></div>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-end gap-px h-full">
            ...
            >
              ...
              <div
                ...
                :style="{ height: `${Math.max(((summary.hour_dist[hour - 1] || 0) / maxHourCount) * 100, 2)}%`, backgroundColor: getTimeSlotColor(hour - 1) }"
              ></div>
            </div>
          </div>
        </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
              <div
                ...
                :style="{ width: `${(cat.count / summary.top_categories[0].count) * 100}%`, backgroundColor: getChartColor(index) }"
              ></div>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div
                  class="h-full rounded-full"
                  :style="{ width: `${(author.count / summary.top_authors[0].count) * 100}%`, backgroundColor: getChartColor(index + 2) }"
                ></div>
              </div>
            </div>
          </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-if="summary.duration_pref?.short + summary.duration_pref?.mid + summary.duration_pref?.long > 0" class="glass-card p-4">
        ...
        <div class="flex h-4 rounded-full overflow-hidden">
          <div class="transition-all" :style="{ width: `${summary.duration_pref.short_ratio * 100}%`, backgroundColor: DURATION_COLORS['短视频'] }" title="短视频"></div>
        ...
        <div class="space-y-2">
          <div v-for="(slot, index) in summary.top_time_slots" :key="slot.name" class="flex items-center gap-2">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full rounded-full" :style="{ width: `${(slot.count / summary.top_time_slots[0].count) * 100}%`, backgroundColor: getChartColor(index + 4) }"></div>
              </div>
            </div>
          </div>
        </div>
...
import { getChartColor, getTimeSlotColor, DEVICE_COLORS, DURATION_COLORS } from '~/utils/chartColors'
