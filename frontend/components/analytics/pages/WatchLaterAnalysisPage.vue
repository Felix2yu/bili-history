...
<template>
  <div class="space-y-6">
    ...
    <div v-if="viewingData" class="text-base text-center text-gray-600 dark:text-gray-300">
      稍后再看列表中有 <span class="text-[#fb7299] font-bold">{{ viewingData.total_count || 0 }}</span> 个视频
    </div>
    ...
    <div v-if="viewingData" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">分类分布</h4>
        <div class="space-y-2">
          <div v-for="(cat, index) in viewingData.category_dist" :key="index"
            ...
            <div class="flex items-center">
              <div class="w-24 h-2 bg-gray-200 dark:bg-gray-700 rounded-full mr-2 overflow-hidden">
                <div class="h-full bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] rounded-full"
                  :style="{ width: `${(cat.count / maxCategoryCount) * 100}%` }"></div>
              </div>
              <span class="text-sm font-medium text-[#fb7299]">{{ cat.count }}</span>
            </div>
          </div>
      ...
      <!-- 时长分布 -->
      <div class="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">时长分布</h4>
        <div class="space-y-2">
          <div v-for="(dur, index) in viewingData.duration_dist" :key="index"
            ...
            <div class="flex items-center">
              <div class="w-24 h-2 bg-gray-200 dark:bg-gray-700 rounded-full mr-2 overflow-hidden">
                <div class="h-full bg-gradient-to-r from-[#fc9b7a] to-[#fb7299] rounded-full"
                  :style="{ width: `${(dur.count / maxDurationCount) * 100}%` }"></div>
              </div>
              <span class="text-sm font-medium text-[#fb7299]">{{ dur.count }}</span>
            </div>
          </div>
      ...
      <div class="lg:col-span-2 bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-gray-300/50 dark:border-gray-500/50">
        <h4 class="text-lg font-bold bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] bg-clip-text text-transparent mb-3">积灰最久的视频</h4>
        <div class="space-y-2">
          <div v-for="(item, index) in viewingData.oldest_items" :key="index"
            ...
            <div class="flex items-center flex-1 min-w-0">
              <span class="w-6 h-6 rounded-full bg-gradient-to-r from-[#fb7299] to-[#fc9b7a] flex items-center justify-center text-white text-xs font-bold mr-2 flex-shrink-0">
                {{ index + 1 }}
              </span>
              <div class="min-w-0">
            ...
            <span class="text-sm font-medium text-[#fb7299] ml-2 whitespace-nowrap">{{ item.days_ago }}天前</span>
          </div>
