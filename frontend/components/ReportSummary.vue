  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-accent">{{ summary.total_videos }}</div>
        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">观看视频数</div>
      </div>
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-accent">{{ formatDurationShort(summary.total_duration) }}</div>
        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">总观看时长</div>
      </div>
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-accent">{{ summary.unique_days }}</div>
        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">活跃天数</div>
      </div>
      <div class="glass-card p-4 text-center">
        <div class="text-2xl font-bold text-accent">{{ summary.unique_authors }}</div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      <div class="glass-card p-4 flex items-center justify-between text-sm">
        ...
        <div class="flex gap-4">
          <span class="text-gray-700 dark:text-gray-300">
            <span class="font-semibold text-accent">{{ summary.avg_daily_videos?.toFixed(1) }}</span> 个视频
          </span>
          <span class="text-gray-700 dark:text-gray-300">
            <span class="font-semibold text-accent">{{ formatDurationShort(summary.avg_daily_duration) }}</span>
          </span>
        </div>
      </div>
      ...
      <div v-if="summary.completion_stats" class="glass-card p-4 flex items-center justify-between text-sm">
        ...
        <div class="flex gap-4">
          ...
          <span class="text-gray-700 dark:text-gray-300">
            <span class="font-semibold text-accent">{{ (summary.completion_stats.avg_rate * 100).toFixed(0) }}%</span> 平均完播率
          </span>
        </div>
      </div>
        ...
        <div class="flex gap-3 flex-wrap">
          ...
          >
            <div
              ...
              :class="device === '手机' ? 'bg-accent' : device === '电脑' ? 'bg-blue-500' : device === '平板' ? 'bg-green-500' : 'bg-gray-400'"
            ></div>
        ...
        <div style="height: 100px;">
          <div class="flex items-end gap-1 h-full">
            ...
            >
              ...
              <div class="w-full mt-auto bg-gradient-to-t from-accent to-accent/70 rounded-t transition-all duration-300 hover:opacity-80"
          ...
          <div class="flex items-end gap-px h-full">
            ...
            >
              ...
              <div
                ...
                :class="(hour - 1) >= 22 || (hour - 1) < 6 ? 'bg-purple-400' : 'bg-accent'"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
              <div
                class="h-full bg-gradient-to-r from-accent to-accent/70 rounded-full"
        ...
        <div class="space-y-1.5">
          <div v-for="day in summary.weekday_dist" :key="day.name" class="flex items-center gap-2">
            ...
            <div class="flex-1 h-4 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
              <div class="h-full bg-gradient-to-r from-accent to-accent/70 rounded-full transition-all duration-300"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div v-if="summary.duration_pref?.short + summary.duration_pref?.mid + summary.duration_pref?.long > 0" class="glass-card p-4">
        ...
        <div class="flex h-4 rounded-full overflow-hidden">
          ...
          <div class="bg-accent transition-all" :style="{ width: `${summary.duration_pref.mid_ratio * 100}%` }" title="中视频"></div>
        ...
        <div class="flex justify-between text-[10px] text-gray-500 dark:text-gray-400 mt-2">
          ...
          <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-accent inline-block"></span>中(5-20min) {{ summary.duration_pref.mid }}</span>
        ...
        <div class="space-y-1.5">
          <div v-for="item in summary.completion_dist" :key="item.range" class="flex items-center gap-2">
            ...
            <div class="flex-1 h-3 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
              <div class="h-full bg-gradient-to-r from-accent to-accent/70 rounded-full" :style="{ width: `${(item.count / maxCompCount) * 100}%` }"></div>
            </div>
        ...
        <div class="space-y-2">
          <div v-for="(slot, index) in summary.top_time_slots" :key="slot.name" class="flex items-center gap-2">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-accent to-accent/70 rounded-full" :style="{ width: `${(slot.count / summary.top_time_slots[0].count) * 100}%` }"></div>
              </div>
            </div>
          </div>
        </div>
